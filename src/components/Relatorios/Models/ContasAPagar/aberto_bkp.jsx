    ...
    return (
      <div className="container mt-4">
        <MDBTable striped bordered hover responsive>
          ...
          <MDBTableBody>
            {mesesUnicos.map((mes, mesIndex) =>
              dados
                .filter((linha) => {
                  // Filtrar dados relevantes para o mês atual
                  const acumulativo = acumulativoMensal.find(
                    (item) => item[0]?.mes === mes
                  );
                  return (
                    acumulativo &&
                    acumulativo[0]?.empresa === linha.empresa &&
                    acumulativo[0]?.revenda === linha.revenda &&
                    acumulativo[0]?.origem === linha.origem
                  );
                })
                .map((linha, linhaIndex) => (
                  <React.Fragment key={`${mes}-${linhaIndex}`}>
                    <tr
                      onClick={() => this.toggleRow(`${mes}-${linhaIndex}`)}
                    ...
                    {expandedRow === `${mes}-${linhaIndex}` && (
                      <tr>
                        <td colSpan={6 + mesesUnicos.length}>
                          <div>
                            <strong>Formas de Pagamento:</strong>
                            <ul>
                              {linha.formas_pagamento?.map(
                                (forma, formaIndex) => (
                                  <li key={formaIndex}>
